...
/**
 * Reads the Groq key, tolerating the two ways it commonly arrives broken:
 * wrapped in quotes (copied straight out of a .env line) or padded with
 * whitespace/newlines (pasted into a dashboard field).
 */
function getApiKey() {
  ...
  if (!raw || !raw.trim()) {
    throw new Error(
      "GROQ_API_KEY is not set. Add it to .env and restart the dev server, or " +
        "add it in Vercel then redeploy — environment variables are only picked " +
        "up by new deployments."
    );
  }
  ...
  if (!key.startsWith("gsk_")) {
    throw new Error(
      "GROQ_API_KEY does not look like a Groq key (it should start with 'gsk_'). " +
        "Check the value was copied without surrounding quotes."
    );
  }
